"""
@file clearance_variability.py
@brief  Clearance Variability evaluator for planning algorithms

@details
Implements  Clearance Variability evaluation based upon the path provided on the grid. 
...
@par Outputs
- @p val:  Clearance Variability computed
...
import math
class ClearanceVariability(BaseEval):
    def __init__(self,pointSamples):
        """
        @brief Construct the class for  Clearance Variability evaluator
    ...
    def eval(self,start,goal,grid,cellSize,path):
        """
        A eval function  for  Clearance Variability evaluation, which evaluates on given start, goal, grid, cellSize, and Path returns  Clearance Variability value
        ...
        @return value Returns the  Clearance Variability
        ...
        if len(path)>=2:
            ...
            self.value=np.var(distances)*cellSize
