"""
@file average_minimum_clearance.py
@brief Average Minimum Clearance evaluator for planning algorithms

@details
Implements Average Minimum Clearance evaluation based upon the path provided on the grid. 
...
@par Outputs
- @p val: Average Minimum Clearance computed
...
class AverageMinimumClearance(BaseEval):
    def __init__(self,pointSamples):
        """
        @brief Construct the class for Average Minimum Clearance evaluator
    ...
    def eval(self,start,goal,grid,cellSize,path):
        """
        A eval function  for Average Minimum Clearance evaluation, which evaluates on given start, goal, grid, cellSize, and Path returns Average Minimum Clearance value
        ...
        @return value Returns the Average Minimum Clearance
        ...
        if len(path)>=2:
            ...
            self.value=np.mean(distances)*cellSize
