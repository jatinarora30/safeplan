from safeplan.main import SafePlan
from safeplan.core.stats import Stats
from safeplan.core.visualize import Visualize

        
sf=SafePlan("/home/jatinarora/safeplan/safeplan/runs/run3.json")
sf.benchmark()


sf=Stats("/home/jatinarora/safeplan/safeplan/runs/run3.json")
sf.compute()

vs=Visualize()
vs = Visualize()
#vs.see("/home/jatinarora/safeplan/safeplan/runs/run1.json", iterNo=1, prefer_plotly=True)

vs.plotGrid("/home/jatinarora/safeplan/safeplan/runs/run3.json", prefer_plotly=True)